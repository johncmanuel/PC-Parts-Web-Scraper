...
class PerCategoryCsvExportPipeline:
    """ Creates a .csv file for each category and its respective data. """
    ...
    def _exporter_for_item(self, item):
        ...
        """ Check if category exists in dictionary. """
        if category not in self.category_to_exporter:
            """ Export scraped data into a csv file with their spider name and category. """
            f = open('data/output/{0}/{1}_{2}.csv'.format(self.spider_name, self.spider_name, category), 'wb')
            exporter = JsonLinesItemExporter(f)
            exporter.start_exporting()
            self.category_to_exporter[category] = exporter
        return self.category_to_exporter[category]
